...
sub footer
{
	...
	<td>$newid</td>
		...
		<td><input type='text' size=20 name=v3></td>
...
sub edit
{
	...
	<td><input type='text' size=20 name=v3 value='$contacts[$targetid]->{MobilePhone}'></td>
	<td><input type='text' size=20 name=v4 value='$contacts[$targetid]->{Email}'></td>
	<td><input type='checkbox' name=o1></td>
	<td><input type='text' size=20 name=v5 value='$contacts[$targetid]->{Add}'></td>
	<td><input type='text' size=20 name=v6 value='$contacts[$targetid]->{Add1}'></td>
...
sub todb
{
	...
	my $query = "INSERT INTO `data`.`table` (`ID`, `FirstName`, `LastName`, `MobilePhone`, `Email`, `Status`, `Sth1`, `Sth2`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
	my $csr = $dbh->prepare($query);
	foreach my $i(@contacts)
	{	
		$csr->bind_param(1,$id);
		$csr->bind_param(2,$i->{FirstName});
		$csr->bind_param(3,$i->{LastName});
		$csr->bind_param(4,$i->{MobilePhone});
		$csr->bind_param(5,$i->{Email});
		if ($i->{Status} eq '+')
		{
			$csr->bind_param(6,'+');
...
sub fromdb
{
	...
	{	
		my $contact={
			FirstName=>"$hash_ref->{FirstName}\n",
			LastName=>"$hash_ref->{LastName}\n",
			MobilePhone=>"$hash_ref->{MobilePhone}\n",
